...
vector<Instr> MultiTranslate(vector<string> strs){
    vector<Instr> res;
    for (int i = 0; i < strs.size(); i++) {
        res.push_back(Instr(strs[i]));
    }
    return res;
}
...
string reasmR(int code){
    ...
    if(rtype==1) return opname+" "+regName[rd]+","+regName[rs]+","+to_string(sa);
...
string reasm(int code){
    ...
    int rs = (code & 0x03E00000)>>21;
    int rt = (code & 0x001F0000)>>16;
    int im = code & 0x0000FFFF;
    
    int dat = (short)im;
    int dot = im & 0x0000FFFF;
    int ofs = (short)im;
