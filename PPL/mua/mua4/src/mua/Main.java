...
public class Main {
    ...
    static class Value{
        ...
        @Override public String toString(){
            ...
            else if(this.type == BasicType.FUNC){
                t = "function";
                v = '[' + val_func.toString() + "]";
            }
            // return t+","+v;
        ...
        public String to_mk_arg(){
            if(type == BasicType.WORD)
                return "\"" + toString();
            return toString();
        }
    }
    ...
    static List op1 = new ArrayList(Arrays.asList(
            "first", "butfirst", "butlast", "save", "load",
            "export", "return", "print", "thing", "not", "erase", "run",
            "isname", "isnumber", "isword", "islist", "isbool", "isempty"));
    static List op2 = new ArrayList(Arrays.asList(
            "sentence", "make", "add", "list", "join",
            "sub", "mul", "div", "mod", "eq", "gt", "lt", "and", "or"));
    ...
    static boolean isArithmeticOp(String s){
        return s.equals("add") || s.equals("sub") || s.equals("mul") || s.equals("div") || s.equals("mod");
    }
    ...
    static Value logic(String op, Value a, Value b){
        ...
        switch (op) {
            case "eq":
                if (a.type != b.type)
                    res.val_bool = false;
                if (a.type != b.type && (a.type == BasicType.LIST || b.type == BasicType.LIST))
                    return res;
                if (a.type == BasicType.WORD)
                    res.val_bool = a.val_word.compareTo(b.val_word) == 0;
                else if (a.type == BasicType.NUMBER)
                    res.val_bool = a.val_num.compareTo(b.val_num) == 0;
                else
                    error("illegal arg type in \"eq\".");
                break;
            case "gt":
                if (a.type != b.type)
                    res.val_bool = false;
    ...
    static String[] brac_close(String[] ops, char left){
        ...
        return opl.toArray(new String[0]);
    }
    ...
    static Value sentence(Value a, Value b){
        String l = "";
        if(a.type == BasicType.LIST)
            l = a.val_ls;
        else if(a.type == BasicType.WORD)
            l = a.val_word;

        String r = "";
        if(b.type == BasicType.LIST)
            r = b.val_ls;
        else if(b.type == BasicType.WORD)
            r = b.val_word;

        return new Value("[" + l.trim() + " " + r.trim() + "]");
    }

    static Value print(Value a){
        String s = a.toString();
        if(a.type == BasicType.LIST)
            s = a.val_ls;
        if(a.type == BasicType.FUNC)
            s = a.val_func.toString();
        System.out.println(s);
        return a;
    }

    static Value list(Value a, Value b){
        String l = a.toString();
        String r = b.toString();
    ...
    static Value join(Value a, Value b){
        if(a.type != BasicType.LIST)
            error("The first arg of `join` must be list!");
        return new Value("[" + a.val_ls + " " + b.toString() + "]");
    }

    static Value first(Value a){
        if(a.type == BasicType.WORD)
            return new Value("\"" + a.val_word.substring(0,1));
        if(a.type == BasicType.LIST){
            List<String> elems = new ArrayList(Arrays.asList(a.val_ls.split(" ")));
            bracket_close(elems, '(');
            bracket_close(elems, '[');
            String start = elems.get(0);
            if(!start.startsWith("[") && !is_num(start))
                start = "\"" + start;
            return new Value(start);
        }
        if(a.type == BasicType.NUMBER)
            return new Value(a.toString().substring(0,1));
        if(a.type == BasicType.BOOL)
            return new Value("\"" + a.toString().substring(0,1));
        return null;
    }

    static Value butfirst(Value a){
        if(a.type == BasicType.WORD)
            return new Value("\"" + a.val_word.substring(1));
        if(a.type == BasicType.LIST){
            List<String> elems = new ArrayList(Arrays.asList(a.val_ls.split(" ")));
            bracket_close(elems, '(');
            bracket_close(elems, '[');
    ...
    static Value butlast(Value a){
        if(a.type == BasicType.WORD)
            return new Value("\"" + a.val_word.substring(0, a.val_word.length()-1));
        if(a.type == BasicType.LIST){
            List<String> elems = new ArrayList(Arrays.asList(a.val_ls.split(" ")));
            bracket_close(elems, '(');
            bracket_close(elems, '[');
    ...
    static Value save(Value a){
        String path = a.toString();
        try{
            FileOutputStream fo = new FileOutputStream(path);
            Map<String, Value> vt = varTables.lastElement();
            List<String> lines = new ArrayList<>();
            for(String name: vt.keySet()){
                String line = "make \"" + name + " " + vt.get(name).to_mk_arg() + "\n";
                lines.add(line);
            }
            for(String ln: lines){
                fo.write(ln.getBytes());
            }
            fo.close();
        }catch (IOException e){
            e.printStackTrace();
    ...
    static Value load(Value a){
        String path = a.toString();
        try{
            File f = new File(path);
            byte[] fctt = new byte[(int)f.length()];
            FileInputStream fi = new FileInputStream(f);
            if(fi.read(fctt) == -1){
                error("end of file: " + path);
            }
            String[] lines =  (new String(fctt)).split("\n");
            fi.close();
            for(String ln: lines){
                parse(ln);
            }
    ...
    static Value erall(){
        for(Map<String, Value> vt: varTables){
            vt.clear();
        }
        return null;
    }
    ...
    static Value parse(String instr){
        ...
        if(args[0].equals("make")){
        ...
        else if(args[0].equals("readlist")){
            argc_chk("readlist", 1, args.length);
            String line = "";
            if(!sc.hasNext())
                error("nothing in input buffer: readlist");
            line = sc.nextLine();
            if(line.equals(""))
                error("empty input: readlist");
            line = "[" + line + "]";
            return new Value(line);
        }
        else if(args[0].equals("sentence")){
            argc_chk("sentence", 3, args.length);
            Value v1 = parse(args[1]);
            assert v1 != null;
            Value v2 = parse(args[2]);
            assert v2 != null;
            return sentence(v1, v2);
        }
        else if(args[0].equals("list")){
            argc_chk("list", 3, args.length);
            Value v1 = parse(args[1]);
            assert v1 != null;
            Value v2 = parse(args[2]);
            assert v2 != null;
            ...
            argc_chk("first", 2, args.length);
            Value v1 = parse(args[1]);
            assert v1 != null;
            return first(v1);
        }
        else if(args[0].equals("butfirst")){
            argc_chk("butfirst", 2, args.length);
            Value v1 = parse(args[1]);
            assert v1 != null;
            return butfirst(v1);
        }
        else if(args[0].equals("butlast")){
            argc_chk("butlast", 2, args.length);
            Value v1 = parse(args[1]);
            assert v1 != null;
            return butlast(v1);
        }
        else if(args[0].equals("save")){
            argc_chk("save", 2, args.length);
            Value v1 = parse(args[1]);
            assert v1 != null;
            return save(v1);
        }
        else if(args[0].equals("load")){
            argc_chk("load", 2, args.length);
            Value v1 = parse(args[1]);
            assert v1 != null;
            return load(v1);
        }
        else if(args[0].equals("erall")){
            argc_chk("erall", 1, args.length);
            return erall();
        }
        else if(args[0].equals("poall")){
            argc_chk("poall", 1, args.length);
            return poall();
        }
